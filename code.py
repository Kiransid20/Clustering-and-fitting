...
def balance_data(df):
  ...
  min_length = min(len(arable_land), len(cereal_yield), len(urban_population))
...
scaler = MinMaxScaler()
transform_data = scaler.fit_transform(df[['arable_land', 'cereal_yield', 'urban_population']])
...
for i in range(5):
    ...
    plt.scatter(cluster_data['arable_land'], cluster_data['cereal_yield'], label=f'Cluster {i}')


plt.scatter(kmeans.cluster_centers_[:, 0], kmeans.cluster_centers_[:, 1], marker='x', c='black', label='Cluster Centers')
plt.xlabel('arable_land')
...
for i in range(5):
    cluster_data = df[df['cluster'] == i]
    plt.scatter(cluster_data['arable_land'], cluster_data['urban_population'], label=f'Cluster {i}')
...
# create a plot showing the clusters and cluster centers using pyplot
for i in range(5):
    cluster_data = swe[swe['cluster'] == i]
    plt.scatter(cluster_data['arable_land'], cluster_data['urban_population'], label=f'Cluster {i}')
